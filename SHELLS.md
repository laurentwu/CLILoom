...
交互终端启动时，CLILoom 会把启动命令立即写入 PTY；TTY 可能先回显一份裸命令，随后 Shell 再绘制提示符和同一命令。主进程会对启动输出做一次有限的流式识别，目标是让终端正文中启动命令只出现一次、绑定变量显示实际参数，同时保留 Shell 提示与程序输出。识别范围是刻意收窄的：

- 识别流开头的完整裸回显，以及紧随其后首条逻辑行中带标准 POSIX 提示符（`$ ` / `# ` 结尾，允许其颜色与标题控制序列）的完整命令重绘。
- Bash 折行长命令时在宽字符前插入的“空格 + 清行”绘制序列（`ESC[K` / `ESC[0K`）会被识别为重绘的一部分。
- macOS 系统 Bash 会识别裸回显后的已知 zsh 迁移提示块及其相邻空行，以及窄列（CR 分段、宽字符重复）重绘。
- Windows Git Bash（ConPTY）可能没有裸回显；CLILoom 会识别其初始化控制前缀（标题、SGR、清屏/光标与模式设置），并规范化首次提示符命令绘制中插入的 bracketed-paste 切换与列边界光标定位。
- 重绘恢复出的字符序列必须与完整启动命令逐字符相等才会替换；识别最多等待 2 秒（自首个 PTY 输出起，不延长），期间未判定的字节按原文释放。
- 其余情况（密码提示、未知 banner、未知控制序列、非列边界定位、重绘不完整、命令后还有额外字符或存在多种可能匹配）一律保留原始输出，不猜测、不丢弃任何字节。超慢 Shell 或未知提示符可能保留重复回显，这是明确的保守回退。

该处理只影响显示与持久化的 transcript，不改变真实执行：命令仍只写入并执行一次，原始 stdout 结果、退出码和交互输入保持原样。这是自动的显示行为，没有面向助手或用户的开关或配置项；`cliloom` 命令也不提供相关设置。
