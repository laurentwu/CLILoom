...
describe('assistant workspace file boundary', () => {
  ...
  it('teaches the new configuration capabilities and resynchronizes guidance for changed builds', () => {
    ...
    expect(instructions).toContain('cliloom workflow get <workflow-id> --json')
    expect(instructions).toContain('cliloom workflow validate')
    expect(instructions).toContain('cliloom workflow save')
    expect(instructions).toContain('--expected-revision')
    expect(instructions).toContain('nodes[].config.autoRetry')
    expect(instructions).not.toContain('auto-retry get')
    expect(instructions).not.toContain('auto-retry set')
