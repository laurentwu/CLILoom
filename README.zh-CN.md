...
- **真实终端执行**
  ...
  - 保存终端输出，并对历史命令进行安全重试。
  - 可在工作流重试或历史重新执行前临时修改命令，仅影响本次执行，不修改工作流及下次默认命令。
  - 终端、Hook、历史重试和 AI CLI 助手共享原生 Shell 目标快照。

- **终端节点失败自动重试**
  - 按节点配置（交互式与非交互式终端），默认关闭，已有工作流不受影响。
  - 推荐设置在每次失败后依次等待 1 → 2 → 5 → 10 → 30 分钟，之后保持每 30 分钟；也可通过表达式助手弹窗生成或直接输入标准五段 Cron（分 时 日 月 周，日/周同时受限时采用 Unix 任一匹配语义）。
  - 次数上限可配置（1～9999 或不限次数）。首次执行和手动重试不计入次数；手动重试开启新一轮计数。
  - 等待中的计划会显示下次重试时间、已重试次数和任务固定时区，并提供“立即重试”和“取消自动重试”；手动停止、Hook 失败和被中断的执行不会安排自动重试。
  - 等待计划在重启和休眠后保留：过期计划在下一次启动或唤醒时只执行一次；任务始终使用启动时绑定的工作流版本和时区。
