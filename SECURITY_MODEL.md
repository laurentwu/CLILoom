...
- Project directories are authorized by a main-process folder dialog, canonicalized, checked as directories, and stored before use. Workflow start accepts the project id as identity and re-reads the database path; a renderer-provided path cannot replace the execution directory. Unsupported `\\wsl$` and `\\wsl.localhost` namespace paths are rejected before use.
...
CLILoom launches only detected native shells as application-managed execution targets. It does not provide WSL discovery, launch, path conversion, environment transport, assistant interop, or session cleanup. This is not a command sandbox: a user-authored terminal command or script can still invoke any executable allowed by the operating system, including `wsl.exe`. Enforce an absolute prohibition with Windows, AppLocker, or organization policy.
