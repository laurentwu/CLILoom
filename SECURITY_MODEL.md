...
Assistant configuration commands ride the same authenticated bridge and keep a closed configuration surface: public settings are limited to an explicit allowlist (active skin, language, both sidebar widths, and the assistant initialization command), workflow writes go through revision-checked saves of complete workflow definitions (which include the terminal automatic-retry configuration) with the designer's unsaved-edit protection, and shell selection only accepts `automatic` or a currently detected candidate id. JSON payloads (workflows and skins) are accepted only from command standard input or a file inside the private assistant workspace; relative paths are validated against directory, parent-traversal, and symlink boundaries. Outputs (including skin exports) are returned as command output rather than writing arbitrary filesystem paths. These commands do not add a generic main-process execution interface and do not change the risk model for user-approved commands: an assistant CLI that the user configured can still read and modify everything the operating-system user can, so the same review discipline applies to configuration changes as to command execution.
